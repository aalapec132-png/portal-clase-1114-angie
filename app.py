...
with app.app_context():
    ...
    if not Usuario.query.filter_by(usuario="henry").first():
        u1 = Usuario(usuario="henry", rol="profesor")
        u1.establecer_contraseña("password123")
        db.session.add(u1)

    # estudiante
    if not Usuario.query.filter_by(usuario="juan").first():
        u2 = Usuario(usuario="juan", rol="estudiante")
        u2.establecer_contraseña("1234")
        db.session.add(u2)
    ...
    if request.method == "POST":
        ...
        if user and user.verificar_contraseña(contraseña):
            ...
            return redirect(url_for(
                "panel_profesor" if user.rol == "profesor" else "panel_estudiante"
            ))
